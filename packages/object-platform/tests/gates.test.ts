...
import { runDemo, runMain } from '../src/cli.js';
...
function createAllowAllTestPolicy(): AuthorizeFn {
  return (req) => ({
    decision: 'allow',
    principalEpids: [],
    resourceEpid: null,
    reason: `test-allow ${req.operation}`,
  });
}

function plat(authorize: AuthorizeFn = createAllowAllTestPolicy()) {
...
describe('Passo 18 gates', () => {
  ...
  it('projetor upsert + history + provenance', async () => {
    const p = plat();
    const m = p.createMapping({
      name: 'm',
      datasetId: 'ds',
    ...
    const r = await p.project({
      mappingVersionId: mv.id,
      datasetVersionId: 'dv-1',
      rows: [{ fields: { id: '1', n: 'x' } }],
    });
    expect(r.upserted).toBe(1);
    const obj = (await p.getObject('alice', r.objectIds[0]!))!;
    expect(obj.properties['pt.n']).toBe('x');
    expect(await p.getHistory('alice', obj.id)).toHaveLength(1);
    expect((await p.getProvenance('alice', obj.id))?.datasetVersionId).toBe('dv-1');
  });

  it('user_edit vence reproject data_source', async () => {
    const p = plat();
    const m = p.createMapping({
      name: 'm',
      datasetId: 'ds',
    ...
    await p.applyUserEdit(id, { 'pt.n': 'user' }, 'alice');
    await p.project({
      mappingVersionId: mv.id,
      datasetVersionId: 'dv-2',
      rows: [{ fields: { id: '1', n: 'source' } }],
    });
    expect((await p.getObject('alice', id))?.properties['pt.n']).toBe('user');
  });

  it('partial does not authorize user_edit', async () => {
    let decision: 'allow' | 'partial' = 'allow';
    ...
    const id = r.objectIds[0]!;
    decision = 'partial';
    await expect(p.applyUserEdit(id, { 'pt.n': 'user' }, 'alice')).rejects.toThrow(
      /deny modify/,
    );
    expect((await p.getObject('alice', id))?.properties['pt.n']).toBe('old');
  });

  it('Object API get/query/traverse/history; deny esconde', async () => {
    ...
    const all = await p.queryObjects('alice', { objectTypeId: 'ot.a' });
    expect(all).toHaveLength(2);
    const child = all.find((o) => o.primaryKey === 'B')!;
    const parents = await p.traverseLinks('alice', child.id);
    expect(parents[0]?.primaryKey).toBe('A');

    deny.add(`object:_/${encodeURIComponent(child.id)}`);
    expect(await p.getObject('alice', child.id)).toBeNull();
    expect(await p.queryObjects('alice', { objectTypeId: 'ot.a' })).toHaveLength(1);
    expect(await p.getHistory('alice', child.id)).toBeNull();
  });
  ...
  it('createObjectPlatform refuses missing authorize', () => {
    expect(() =>
